...
/**
 * Customized version of ___
 *
 * As suggested at the official link, this is a copy of WP_List_Table class
 * to use and distribute with the plugin
 *
 * @link https://developer.wordpress.org/reference/classes/wp_list_table/#developer-usage-private-status
 * @since 3.1.0
 * @access private
 */
class Fitet_Monitor_Wp_Table {
	...
	public function search_box($text, $input_id) {
		...
		$esc_attr = esc_attr($input_id);
		$admin_search_query = $this->_admin_search_query();
		$get_submit_button = get_submit_button($text, '', '', false, array('id' => 'search-submit'));
	...
	protected function bulk_actions($which = '') {
		...
		foreach ($this->_actions as $key => $value) {
			if (is_array($value)) {
				$b .= "\t" . '<optgroup label="' . esc_attr($key) . '">' . "\n";

				foreach ($value as $name => $title) {
					$class = ('edit' === $name) ? ' class="hide-if-no-js"' : '';
					$b .= "\t\t" . '<option value="' . esc_attr($name) . '"' . $class . '>' . $title . "</option>\n";
				}
				$b .= "\t" . "</optgroup>\n";
			} else {
				$class = ('edit' === $key) ? ' class="hide-if-no-js"' : '';
				$b .= "\t" . '<option value="' . esc_attr($key) . '"' . $class . '>' . $value . "</option>\n";
			}
		}
	...
	protected function months_dropdown($post_type) {
		...
		foreach ($months as $arc_row) {
			...
			$filter_by_date = $get_post_type_object->labels->filter_by_date;
			...
			$html = "<label for='filter-by-date' class='screen-reader-text'>$filter_by_date</label>
					 <select name='m' id='filter-by-date'>
					 	<option $selected value='0'>$var</option>
					 	$options
					 </select>";
		}
		return $html;
	...
	protected function view_switcher($current_mode) {
		$esc_attr = esc_attr($current_mode);

		$html = '<input type="hidden" name="mode" value="' . $esc_attr . '"/>';
	...
	public function display() {
		...
		$display_tablenav_top = $this->display_tablenav('top');
		$tag = $this->screen->render_screen_reader_content('heading_list');
		$tag_class = implode(' ', $this->get_table_classes());
		$print_column_headers_top = $this->print_column_headers();
		$data = ($singular) ? " data-wp-lists='list:$singular'" : '';
		$display_rows_or_placeholder = $this->display_rows_or_placeholder();
		$print_column_headers_bottom = $this->print_column_headers(false);
		$display_tablenav_bottom = $this->display_tablenav('bottom');

		return "$tag
		$display_tablenav_top
		<table class='wp-list-table $tag_class'>
			<thead>
			<tr>$print_column_headers_top</tr>
			</thead>
			<tbody id='the-list' $data>
			$display_rows_or_placeholder
			</tbody>
			<tfoot>
			<tr>$print_column_headers_bottom</tr>
			</tfoot>
		</table>
		$display_tablenav_bottom";
	...
	protected function display_tablenav($which) {
		if ('top' === $which) {
			$wp_nonce = wp_nonce_field('bulk-' . $this->_args['plural'], '_wpnonce', true, false);
		}
		$esc_attr = esc_attr($which);
		$has_items = $this->has_items();
		$bulk_actions = '';
		if ($has_items)
			$bulk_actions = '<div class="alignleft actions bulkactions">' . $this->bulk_actions($which) . '</div>';
		$extra_tablenav = $this->extra_tablenav($which);
		$pagination = $this->pagination($which);

		return "<div class='tablenav $esc_attr'>
		$bulk_actions
		$extra_tablenav
		$pagination
		<br class='clear'/>
	...
	public function display_rows_or_placeholder() {
		if ($this->has_items()) {
			...
			return '<tr class="no-items"><td class="colspanchange" colspan="' . $this->get_column_count() . '">' .
				$this->no_items() .
				'</td></tr>';
		}
	}
	...
	public function single_row($item) {
		return '<tr>' . $this->single_row_columns($item) . '</tr>';
	}
		...
		foreach ($columns as $column_name => $column_display_name) {
			...
			if ('cb' === $column_name) {
				...
				$html .= "<td $attributes>" .
					...
					$this->handle_row_actions($item, $column_name, $primary) .
					'</td>';
			} else {
				$html .= "<td $attributes>" .
