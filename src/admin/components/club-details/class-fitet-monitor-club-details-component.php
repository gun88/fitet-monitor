...
class Fitet_Monitor_Club_Details_Component extends Fitet_Monitor_Component {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(get_class($this), 'wpApiSettings', ['nonce' => wp_create_nonce('wp_rest')]);
    }
    ...
    public function player_table($players) {
        ...
        return $this->components['table']->render([
            ...
            'rows' => array_map(function ($player) {
                ...
                $player['image'] = $this->images($player);;
                return $player;
            }, $players),
        ]);
    }
    ...
    private function images($player) {
        $playerId = $player['playerId'];
        $label = __('Load', 'fitet-monitor');
        $labelDefault = __('Default', 'fitet-monitor');
        return '
<div class="fm-uploader">
  <input type="file" data-code="' . $playerId . '" class="fm-file" accept="image/png,image/jpeg" hidden>
  <button type="button" class="fm-upload-btn" aria-label="' . $label . '">
    <span class="fm-emoji" aria-hidden="true">📤</span>
    <span class="fm-text">' . $label . '</span>
  </button>
  <button type="button" class="fm-default-btn" data-code="' . $playerId . '" aria-label="' . $labelDefault . '">
    <span class="fm-emoji" aria-hidden="true">❌</span>
    <span class="fm-text">' . $labelDefault . '</span>
  </button>
  <span class="fm-status" role="status" aria-live="polite"></span>
</div>';
    }
