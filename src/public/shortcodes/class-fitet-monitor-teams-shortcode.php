...
class Fitet_Monitor_Teams_Shortcode extends Fitet_Monitor_Shortcode {
    ...
    protected function attributes() {
        return ['club', 'team', 'season', 'championship', 'mode', 'players-page-id', 'matches-page-id'];
    }
    ...
    public static function extract_championships($clubs) {
    ...
    private function single($attributes) {
        ...
        $resources = Fitet_Monitor_Teams_Shortcode::extract_championships($resources);

        $resources = $this->filter_championships($resources, $attributes['season'], $attributes['championship']);
        $to_match_groups = Fitet_Monitor_Utils::to_match_groups([$resources], false, 'championshipDayId', $attributes['matches-page-id']);
        ...
        $resources[0]['calendar'] = $to_match_groups;
    ...
    private function list($attributes) {
        ...
        if (empty($attributes['season'])) {
            $attributes['season'] = Fitet_Monitor_Utils::last_season_id($resources);
        }
        ...
        $resources = Fitet_Monitor_Teams_Shortcode::extract_championships($resources);
        $seasons = Fitet_Monitor_Utils::extract_seasons($resources);
