...
class Fitet_Monitor_Team_Image_Component extends Fitet_Monitor_Component {
	...
	protected function process_data($data) {
		$data = array_merge(['clubCode' => '', 'teamName' => 'N/A', 'teamPageUrl' => '', 'clubLogo' => ''], $data);
		$club_code = $data['clubCode'];
		$team_name = $data['teamName'];
		$team_image_url = $data['clubLogo'];

		if (empty($team_image_url)) {
			if (empty($club_code)) {
				$team_image_url = FITET_MONITOR_CLUB_NO_LOGO;
			} else {
				$team_image_url = "http://portale.fitet.org/images/societa/$club_code.jpg";
			}
		}
