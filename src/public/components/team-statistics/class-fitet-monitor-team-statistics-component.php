...
class Fitet_Monitor_Team_Statistics_Component extends Fitet_Monitor_Component {
    ...
    private function rows($data) {
        ...
        $players = array_map(function ($player) use ($players) {
            $player['playerPageUrl'] = $player['playerUrl'];
            $player['playerImage'] = $player['playerImageUrl'];
        ...
        return array_map(function ($player) use ($players) {
            $p = Fitet_Monitor_Matches_Shortcode::try_parse_double_inner($players, $player);
            $player['playerName'] = $this->components['playerCell']->render($p);
            return $player;
        }, $players);
    }
